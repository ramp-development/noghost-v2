...
(() => {
  ...
  // node_modules/.pnpm/file+gsap-bonus.tgz/node_modules/gsap/ScrambleTextPlugin.js
  ...
  var ScrollTrigger2 = /* @__PURE__ */ function() {
    ...
    _proto.init = function init5(vars, animation) {
      ...
      !animation || !animation.add || change ? self.refresh() : gsap5.delayedCall(0.01, function() {
  ...
  // src/utils/onAllImagesLoaded.ts
  var onAllImagesLoaded = (callback) => {
    window.onload = () => {
  ...
  // src/utils/resizeHero.ts
  var resizeHero = () => {
    const attr = "data-component";
    const banner2 = queryElement(`[${attr}="banner"]`);
    const nav = queryElement(`[${attr}="nav"]`);
    const heros = queryElements(`[${attr}="hero"]`);
    if (heros.length === 0)
      return;
    for (const hero of heros) {
      let offsetHeight = 0;
      if (banner2)
        offsetHeight += banner2.offsetHeight;
      if (nav)
  ...
  // src/animations/banner.ts
  var banner = () => {
    console.log("banner");
    if (sessionStorage.getItem("bannerClosed") !== null)
      return;
  ...
  // src/animations/utils/launchConfetti.ts
  var launchConfetti = () => {
    const duration = 1 * 1e3;
  ...
  // src/animations/celebration.ts
  var celebration = () => {
    console.log("celebration");
    const trigger = queryElement('[data-confetti="trigger"]');
    if (!trigger)
      return;
    trigger.addEventListener("click", () => {
      launchConfetti();
  ...
  // src/animations/utils/getRandomColor.ts
  var getRandomColor = () => {
  ...
  // src/animations/scramble.ts
  var createScrambleAnimation = (element) => {
    const text = element.textContent;
    if (!text)
      return gsapWithCSS.timeline();
    element.style.minWidth = `${element.offsetWidth}px`;
    const elementTimeline = gsapWithCSS.timeline({
      paused: true,
      onComplete: () => {
    ...
    splitElement.chars.forEach((char) => {
      if (!char.textContent)
        return;
      const charTimeline = gsapWithCSS.to(char, {
        duration: 0.5,
        onStart: () => {
  ...
  var scramble = () => {
    ...
    const elements = queryElements("[data-scramble]");
  ...
  var dream = () => {
    ...
    function moveXItemsToEnd(arr, x) {
      if (arr.length <= x)
        return arr.slice();
  ...
  var watchElementsForClassListChanges = (elements, callback) => {
    const observer = new MutationObserver(callback);
    const config3 = { attributes: true, attributeFilter: ["class"] };
    elements.forEach((element) => {
      observer.observe(element, config3);
    });
    return observer;
  };

  // src/pages/about/timeline.ts
  var timeline2 = () => {
    console.log("timeline");
    const timeline3 = queryElement(".timeline_component");
    const anchors = queryElements(".timeline_anchor", timeline3);
    const items = queryElements(".timeline_right-item", timeline3);
    ...
    watchElementsForClassListChanges(anchors, (mutations) => {
      mutations.forEach((mutation) => {
        ...
        if (!target.classList.contains("w--current"))
          return;
        makeAllItemsInactive();
        const index2 = anchors.indexOf(target);
    ...
    function setActiveItem(index2) {
      items[index2].classList.add("is-active");
    }
    function moveAnchors(index2) {
      anchors.forEach((anchor) => {
        anchor.style.transform = `translateY(-${index2 * 100}%)`;
      });
    }
  };
  ...
  var careers = () => {
    console.log("careers");
    const accordionLists = queryElements(".accordion_list");
    accordionLists.forEach((accordionList) => {
      const accordionHeader = queryElement(".accordion_header", accordionList);
      if (accordionHeader)
  ...
  var work = () => {
    console.log("work");
    const parents = queryElements("[data-tags-parent]");
    const children = queryElements("[data-tags-child]");
    parents.forEach((parent) => {
      const child = children.find((child2) => child2.dataset.tagsChild === parent.dataset.tagsParent);
      if (!child)
        return;
  ...
  var workTemplate = () => {
    ...
    const component = queryElement(`[${attr}="component"]`);
    const items = queryElements(`[${attr}="item"]`, component);
    const groups = queryElement(`[${attr}="groups"]`, component);
    const groupTemplate = queryElement(`[${attr}="group-template"]`, groups);
    if (!component || items.length === 0 || !groups || !groupTemplate)
      return;
    const groupNames = [];
    items.forEach((item) => {
      const creditsGroup = item.dataset.creditsGroup || "";
      if (groupNames.includes(creditsGroup))
        return;
      groupNames.push(creditsGroup);
    });
    groupNames.forEach((groupName) => {
      const groupClone = groupTemplate.cloneNode(true);
      const groupTitle = queryElement(`[${attr}="group-title"]`, groupClone);
      if (groupTitle && groupName !== "") {
        groupTitle.textContent = groupName;
      } else {
        groupTitle?.remove();
      }
      const groupList = queryElement(`[${attr}="group-list"]`, groupClone);
      if (groupList) {
        items.forEach((item) => {
          if (item.dataset.creditsGroup !== groupName)
            return;
    ...
    function toggleView(groups2, button2) {
      const hasClass = groups2.classList.contains("is-open");
      if (hasClass) {
        updateButtonText(button2, "View More");
      } else {
        updateButtonText(button2, "View Less");
      }
      groups2.classList.toggle("is-open");
    }
    function updateButtonText(button2, text) {
      const buttonTexts = queryElements(".button-text", button2);
      buttonTexts.forEach((buttonText) => {
        buttonText.textContent = text;
      });
    }
    const collection = queryElement(`[${attr}="collection"]`, component);
    if (collection)
      collection.remove();
  };
  ...
  window.Webflow ||= [];
  window.Webflow.push(() => {
    gsapWithCSS.registerPlugin(Flip, ScrambleTextPlugin, ScrollTrigger2, SplitText);
    animations();
    pages();
    onAllImagesLoaded(() => ScrollTrigger2.refresh());
  });
})();
